...
export default function EventEditPage({ onUpdated }) {
  const { id } = useParams();
  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        const uniq = (arr) => Array.from(new Set((arr||[]).filter(Boolean))).sort((a,b)=>String(a).localeCompare(String(b),"ko"));
  ...
  const hostCandidates = useMemo(() => {
    if (!form) return [];
    const list = (users||[]).filter(u => form.department ? u.department === form.department : true);
  ...
  const requiredOk = form.event_name && form.start_date && form.end_date && form.department;
...
const onSubmit = async (e) => {
  ...
  try {
    ...
    const { error } = await supabase
      .from("events")
      .update(payload)
      .eq("id", id);   // ✅ 그냥 id 그대로 (UUID string)
        ...
        <form onSubmit={onSubmit} className="bg-white rounded-xl border p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input name="host" value={form.host} onChange={onChange} className="w-full border rounded px-3 py-2" list="hostOptions"/>
              <datalist id="hostOptions">
                {hostCandidates.map(n=><option key={n} value={n} />)}
              </datalist>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            <button type="submit" disabled={!requiredOk || invalidEnd} className="px-4 py-2 bg-black text-white rounded">저장</button>
